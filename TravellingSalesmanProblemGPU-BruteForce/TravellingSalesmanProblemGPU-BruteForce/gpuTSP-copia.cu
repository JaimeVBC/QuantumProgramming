...
int main(int argc, char *argv[])
{
	...
	int numNodes = atoi(argv[1]), *cost; //Number of cities to be computed
	int* edgesWeights;
	...
	SAFE(edgesWeights = (int*)malloc(numNodes * sizeof(int) * numNodes));
	SAFE(cost = (int*)calloc(num_blocks * numNodes, sizeof(int)));
	...
	CUDA_RUN(cudaMalloc((void **)&d_edgesWeights, numNodes * sizeof(int) * numNodes));
	CUDA_RUN(cudaMalloc((void **)&d_costs, num_blocks * sizeof(int)));
	...
	CUDA_RUN(cudaMemcpy(d_edgesWeights, edgesWeights, numNodes * sizeof(int) * numNodes, cudaMemcpyHostToDevice));
	...
	CUDA_RUN(cudaMemcpy(d_costs, cost, num_blocks * sizeof(int), cudaMemcpyHostToDevice));
	...
	CUDA_RUN(cudaMemcpy(cost, d_costs, num_blocks * sizeof(int), cudaMemcpyDeviceToHost));
...
__global__
void find_permutations_for_threads(int8_t * nodes_ids, int8_t * k, int8_t * choices, int * size, unsigned long long * threads_per_kernel) {
	int length = *size;
	int8_t index = 1;
...
__global__
void combinations_kernel(int8_t * choices, int8_t * k, int8_t * shortestPath, int* graphWeights, int* cost, int * size) {
...
__host__
void initialize(int8_t * nodes_ids, int* nodesDistances, int size)
{
	...
	for (int row = 0; row < size; ++row)
	{
		...
		for (int col = 0; col < size; ++col)
		{
			...
			convertor >> temp;
			//printf("%f\t",temp);
			nodesDistances[row*size+col] = (int)(temp*10000);
...
__host__
void print_Graph(int* nodesDistances, int size) {
	int i, j;
	for (i = 0; i < size; i++) {
		for (j = 0; j < size; j++) {
...
__host__
void print_ShortestPath(int8_t * shortestPath, int cost, int size) {
	int i;
...
__device__
void copy_array2(int8_t * path, int8_t * shortestPath, int* tcost, int* weights, int8_t length, int tid) {
	int sum = 0;
	for (int i = 0; i < length; i++) {
